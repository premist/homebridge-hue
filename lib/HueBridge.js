  ...
  for (const key in obj.resourcelinks) {
    ...
    if (
      link.name === 'homebridge-hue' && link.links && link.description && (
        ...
        link.owner === this.hueClient.username
      )
    ) {
      ...
      for (const resource of link.links) {
        ...
        if (list === 'multiclip') {
          ...
          this.valve[id] = true
        } else if (list === 'venetianblind') {
          if (type !== 'lights') {
            this.log.warn(
              '%s: /resourcelinks/%d: %s: ignoring unsupported venetianblind resource',
              this.name, key, resource
            )
            continue
          }
          this.venetianblind[id] = true
...
HueBridge.prototype.sensorSerialNumber = function (id, obj) {
  ...
  if (obj.type[0] === 'Z') {
    ...
    if (a != null) {
      ...
      if (this.platform.config.hueMotionTemperatureHistory) {
        ...
        if (
          obj.manufacturername === this.philips && (
            ...
            obj.modelid === 'SML003' || obj.modelid === 'SML004'
          )
        ) {
          // Hue motion sensor.
      ...
      if (
        obj.manufacturername === 'Develco Products A/S' &&
...
HueBridge.prototype.isLightSensor = function (id, obj) {
...
HueBridge.prototype.listen = function () {
  ...
  ws
    ...
    .on('listening', (url) => {
      this.log('%s: websocket connected to %s', this.name, url)
    })
...
HueBridge.prototype.setSearch = function (search, callback) {
  ...
  const promise = this.isDeconz
    ? this.put('/config', { permitjoin: 120 })
    : this.post('/lights')
  const timeout = this.isDeconz ? 120 : 60
  if (search) {
    promise.then(() => {
...
HueBridge.prototype.post = async function (resource, body) {
  try {
